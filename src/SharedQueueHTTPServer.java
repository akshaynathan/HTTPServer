...
public class SharedQueueHTTPServer {
	...
	public static void main(String args[]) {
		...
		try {
			...
			SharedQueueThread[] threads = new SharedQueueThread[threadCount];
			for(int i = 0; i < threads.length; i++) {
				threads[i] = new SharedQueueThread(pool, documentRoot, "SharedQueueHTTPServer", cache);
				threads[i].start();
			}
			while(true) {
